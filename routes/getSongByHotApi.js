...
router.get('/getSongByHotApi', function (req, res) {
    console.log('热门推荐');
            ...
            if (!err) {
                ...
                db.collection(targetCollection, function (err, collection) {
                    if (err) {
                        ...
                        //find
                        collection.find().sort({"listener":-1,"collectNum":-1,"timeStamp":-1}).toArray(function (err, docs) {
                            if (err) {
                                throw err;
                            }
                            else {
    ...
    queryData('allSongs',function (callback) {
